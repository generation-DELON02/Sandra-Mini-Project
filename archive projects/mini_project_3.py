    ...
    while(True):
        ...
        while(True):
            ...
            if user_input_drink ==0:
                ...
                continue
            break
        list_of_drink.append(user_input_drink)

        keep_going = input("Would you like to add another drink?(Y/N)")
        if cont.upper() == "N":
            drinks_menu()
            break
        elif cont.upper() == "Y":
            ...
            print('Sorry Input invalid, going back to the drinks menu')
            drinks_menu()
            break
        Order_list.append({
        'customer_name': name, 
...
def save_orders():
    with open('order_list.csv','w') as g:
        fieldnames = ['customer_name', 'customer_address', 'customer_phone', 'courier', 'status', 'drinks']
